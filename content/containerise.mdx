...
Zenith Hosting deploys prebuilt container images. We do not build your source code during deployment, so you need to publish a container image for each service your app needs.

<Info>
    If you already maintain a container image that can be pulled without authentication, skip this step and go straight to [creating `zenith-compose.yml`](/content/zenith-compose).
</Info>
...
<Columns cols={2}>
  <Card title="Build and share an image" icon="cube" href="https://docs.docker.com/get-started/tutorials/run-an-app/">
...
- Read deployment-specific values and secrets from environment variables; use [inline Compose configs](/content/zenith-compose#add-static-configuration) for static, non-secret files
- Write logs to `stdout` and `stderr`
- Listen on `0.0.0.0`, not `localhost`
- Store any persistent data in a documented directory
- Start using only the container image, declared environment variables, config files, and volumes
...
Publish the image to a registry that allows anonymous pulls. For GitHub projects, GHCR is the simplest option. Docker Hub is also a good option.
...
<Columns cols={2}>
  ...
  <Card title="Publish to Docker Hub" icon="docker" href="https://docs.docker.com/docker-hub/repos/manage/hub-images/push/">
