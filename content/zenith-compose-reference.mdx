...
Unknown fields inside `x-zenith` fail parsing. Standard Compose fields are parsed separately; only the fields listed under [Compose compatibility](#compose-compatibility) have defined Zenith runtime behavior, others will be ignored.
...
| `x-zenith.configs` | list | No | Zenith configuration metadata. |
...
`x-zenith.configs` accepts a list of configuration-file descriptors. This is separate from the top-level Compose `configs` map.
...
This is Zenith metadata only. It does not mount, edit, or transform the referenced file during deployment.
